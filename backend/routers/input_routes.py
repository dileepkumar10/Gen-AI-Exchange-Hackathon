...
async def enhanced_analysis(username, file_path, query, progress_tracker=None):
...
async def parse_uploaded_files(
    ...
    try:
        ...
        if AnalysisProgressTracker and websocket_manager:
        ...
        # Step 1: Skip document processing for demo, go directly to LLM analysis
        if progress_tracker:
            await progress_tracker.next_step(confidence=0.95)
            
        structured_data = {'company_info': {}, 'financial_data': {}, 'team_info': {}, 'market_data': {}}
        print("Skipping document processing, proceeding to LLM analysis")
        
        # Step 2: Market Intelligence
        if progress_tracker:
            await progress_tracker.next_step(confidence=0.87)
        ...
        # Step 3: Direct LLM Analysis (bypass document processing)
        if progress_tracker:
            await progress_tracker.next_step(confidence=0.92)
        
        print("Starting direct LLM analysis...")
        analysis_tasks = [
            enhanced_analysis(current_user.username, file_path, "founders_profile", progress_tracker),
            enhanced_analysis(current_user.username, file_path, "market_problem", progress_tracker),
            enhanced_analysis(current_user.username, file_path, "unique_differentiator", progress_tracker),
            enhanced_analysis(current_user.username, file_path, "business_metrics", progress_tracker),
            enhanced_analysis(current_user.username, file_path, "risk_factor", progress_tracker),
            enhanced_analysis(current_user.username, file_path, "recommended_next_steps", progress_tracker)
        ]
        ...
        if progress_tracker:
            await progress_tracker.next_step(confidence=0.89)
        ...
        # Complete analysis
        if progress_tracker:
            await progress_tracker.complete(final_results)
        ...
        # Send error via WebSocket if progress tracker exists
        try:
            if 'progress_tracker' in locals() and progress_tracker:
                await progress_tracker.error(f"Analysis failed: {str(e)}")
        except:
            pass
