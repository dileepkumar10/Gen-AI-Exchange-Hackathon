...
const AIChatInterface = ({ analysisResults, isOpen, onToggle }) => {
  ...
  return (
    <motion.div
      ...
      className={`fixed right-6 bottom-6 flex flex-col bg-white/10 backdrop-blur-md bg-black rounded-xl border border-white/20 shadow-2xl z-50 overflow-hidden ${
        isMinimized ? 'w-80 h-16' : 'w-[95vw] md:w-[450px] max-h-[85vh] h-[600px]'
      } transition-all duration-300 max-w-full`}
    >
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-white/20 bg-black/20 backdrop-blur-md sticky top-0 z-10">
        <div className="flex items-center space-x-3">
          <div className="w-8 md:w-10 h-8 md:h-10 bg-gradient-to-r from-purple-500 to-pink-500 rounded-full flex items-center justify-center">
            <Bot className="w-4 md:w-5 h-4 md:h-5 text-white" />
          </div>
          <span className="text-white font-medium text-base md:text-lg">AI Assistant</span>
        </div>
        <div className="flex items-center space-x-3">
          <button
      ...
      {!isMinimized && (
        <>
          {/* Messages */}
          <div className="flex-1 overflow-y-auto overscroll-y-contain px-3 md:px-4 py-4 space-y-4 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
            <AnimatePresence>
              ...
              <div className="flex flex-wrap gap-1.5 md:gap-2">
                {quickPrompts.slice(0, 3).map((prompt) => (
                  <button
                    ...
                    className="text-[10px] md:text-xs bg-white/10 hover:bg-white/20 text-white px-2 py-1 rounded-full transition-colors truncate max-w-[150px]"
                  >
                    {prompt}
                  </button>
                ))}
          ...
          {/* Input */}
          <div className="px-3 md:px-4 py-3 md:py-4 border-t border-white/20 bg-black/20 backdrop-blur-md sticky bottom-0">
            <div className="flex items-center space-x-2 md:space-x-3">
              <input
                ...
                className="flex-1 bg-white/10 border border-white/20 rounded-lg px-3 md:px-4 py-2 md:py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 text-xs md:text-sm min-h-[40px] md:min-h-[44px] w-full"
                disabled={isLoading}
              />
              <button
                onClick={handleSendMessage}
                disabled={!inputValue.trim() || isLoading}
                className="bg-purple-500 hover:bg-purple-600 text-white p-2 md:p-3 rounded-lg disabled:opacity-50 transition-colors flex items-center justify-center min-w-[40px] md:min-w-[44px] min-h-[40px] md:min-h-[44px] flex-shrink-0"
              >
