...
import { TrendingUp, Users, Lightbulb, DollarSign, BarChart3, Moon, Sun } from 'lucide-react';
import DataVisualization from './DataVisualization';
import AIChatInterface from './AIChatInterface';
import { useTheme } from './ThemeProvider';
import { Switch } from './ui/Switch';

const EnhancedAnalysisDashboard = ({ analysisResults, isAnalyzing, currentStep }) => {
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="flex justify-end">
          <div className="flex items-center space-x-2 bg-white/10 backdrop-blur-md rounded-lg p-2">
        ...
        >
          ...
          <h2 className="text-2xl font-bold text-white mb-4">AI Agents Working...</h2>
              ...
              <div key={index} className="flex items-center">
                <motion.div
                  ...
                  className={`flex flex-col items-center p-4 rounded-lg min-w-[120px] ${
                    index === currentStep ? 'bg-purple-500/20 border border-purple-500/30' : 
          ...
          <motion.div
            initial={{ width: 0 }}
            animate={{ width: `${((currentStep + 1) / aiSteps.length) * 100}%` }}
            className="h-2 bg-gradient-to-r from-purple-500 to-pink-500 rounded-full mx-auto"
            style={{ maxWidth: '300px' }}
          />
          <p className="text-purple-200 mt-2">
            Step {currentStep + 1} of {aiSteps.length} - {Math.round(((currentStep + 1) / aiSteps.length) * 100)}% Complete
          </p>
        </motion.div>
      </div>
    );
