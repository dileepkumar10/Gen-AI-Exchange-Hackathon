...
class MemoGenerationAgent {
  ...
  listDataSources(inputData) {
    const sources = [];
    if (inputData.documents) sources.push('Pitch deck documents');
    if (inputData.voice) sources.push('Voice recordings');
    if (inputData.publicData) sources.push('Public market data');
    return sources;
  }

  describeMethodology() {
    return {
      approach: 'Multi-agent AI analysis',
      scoring: 'Weighted composite scoring (0-100)',
      validation: 'Cross-agent validation and consensus',
  ...
  listAssumptions() {
    return [
      'Market data accuracy within 6 months',
      'Financial projections based on provided data',
      'Competitive landscape analysis current as of analysis date',
...
export default MultiAgentSystem;ch deck and documents');
